...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_vcf", help="Output cravat-annotated vcf file", required=True)

    parser.add_argument("--genome", help="genome (hg19 or hg38)", default='hg38')
    parser.add_argument("--cravat_lib_dir", help="cravat resources lib dir", required=True)
    ...
    if count != 1:
        raise RuntimeError("Error, couldnt replace .vcf extension of {}".format(output_vcf))
    
    # set cravat resource lib:
    subprocess.check_call(["oc", "config", "md", cravat_lib_dir])
    
    # run open-cravat
    cravat_cmd = ['oc', 'run', '--cs', '{"run":{"vcfreporter":{"type":"separate"}}}',
                  input_vcf,
                  '-t', 'vcf',
                  '-l', args.genome,
                  '-d', output_dir,
